...
async function restore(table:"projects"|"designs"|"experiments"|"services"|"pricing_packages"|"testimonials",id:string,extra:Record<string,unknown>={}){const u=await admin(),s=await createClient();await s.from(table).update({deleted_at:null,updated_by:u.id,updated_at:new Date().toISOString(),...extra}).eq("id",id);revalidatePath("/admin/trash")}
...
export async function deletePricingForever(fd:FormData){await destroy("pricing_packages",String(fd.get("id")))}\nexport async function restoreTestimonial(fd:FormData){await restore("testimonials",String(fd.get("id")),{visibility:false});revalidatePath("/admin/testimonials");revalidatePath("/")}\nexport async function deleteTestimonialForever(fd:FormData){await destroy("testimonials",String(fd.get("id")))}